...
if ! echo "$2" | grep -q -E '^[a-z][0-9a-z-]{,63}$'; then
...
BASE_IMAGE_NAME="ubuntu/noble/cloud"
HEADER=$(head "${PLATFORM_FILE}" -n 1)
if echo "${HEADER}" | grep -q -E '^\s*#\s+base-image:\s+[a-z0-9/-]+\s*$'; then
  BASE_IMAGE_NAME=$(echo "${HEADER}" | cut -d\  -f3)
fi
...
"${INCUS_COMMAND}" init images:"${BASE_IMAGE_NAME}" "${INSTANCE_NAME}" <&4
